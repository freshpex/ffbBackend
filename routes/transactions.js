...
router.post("/withdrawal", async (req, res) => {
  ...
  try {
    ...
    if (method === "cryptocurrency" && !walletAddress) {
      return res
        .status(400)
        .json({ message: "Wallet address is required for crypto withdrawals" });
    }

    if (method === "bank_transfer" && !bankDetails) {
      return res
        .status(400)
        .json({ message: "Bank details are required for bank transfers" });
    }

    if (method === "paypal" && !paypalEmail) {
      return res
        .status(400)
        .json({ message: "PayPal email is required for PayPal withdrawals" });
    }
