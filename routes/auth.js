...
router.post('/register', async (req, res) => {
  try {
    ...
    logger.info(`User registration request: ${email} | Firebase UID: ${uid ? uid.substring(0, 8) + '...' : 'not provided'}`);
    
    // Validate required fields
    if (!uid || !email || !firstName || !lastName) {
      logger.warn(`Registration failed: Missing required user information | Email: ${email || 'not provided'}`);
    ...
    if (referrer) {
      ...
      try {
        ...
        logger.error(`Error creating referral record: ${referralError.message}`);
