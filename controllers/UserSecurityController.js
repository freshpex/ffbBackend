...
export const changePassword = async (req, res, next) => {
  try {
    ...
    if (!currentPassword || !newPassword) {
      throw new ApiError('Current password and new password are required', 400, 'validation_error');
    }
    
    if (newPassword.length < 8) {
      throw new ApiError('New password must be at least 8 characters long', 400, 'validation_error');
    }
...
export const verify2FA = async (req, res, next) => {
  try {
    ...
    if (!token) {
      throw new ApiError('Verification token is required', 400, 'validation_error');
    }
...
export const disable2FA = async (req, res, next) => {
  try {
    ...
    if (!token || !password) {
      throw new ApiError('Verification token and password are required', 400, 'validation_error');
    }
...
export const getSecurityStatus = async (req, res, next) => {
  try {
    const user = await User.findById(req.user._id)
      .select('twoFactorAuth passwordChangedAt email emailVerified phone phoneVerified');
