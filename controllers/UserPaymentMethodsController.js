...
export const addBankAccount = async (req, res, next) => {
  try {
    const { accountName, accountNumber, bankName, routingNumber, bankAddress, swiftCode, nickname } = req.body;
    
    // Validate required fields
    if (!accountName || !accountNumber || !bankName) {
      throw new ApiError('Account name, account number, and bank name are required', 400, 'validation_error');
    }
...
export const addCryptoWallet = async (req, res, next) => {
  try {
    ...
    if (!cryptocurrency || !walletAddress) {
      throw new ApiError('Cryptocurrency type and wallet address are required', 400, 'validation_error');
    }
...
export const setDefaultPaymentMethod = async (req, res, next) => {
  ...
  try {
    ...
    logger.error(`Error setting default payment method ${req.params.id}:`, error);
    next(error);
  } finally {
    session.endSession();
  }
