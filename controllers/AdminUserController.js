...
export const createUser = async (req, res, next) => {
  ...
  try {
    ...
    if (!email || !firstName || !lastName || !password) {
      throw new ApiError('Email, firstName, lastName and password are required', 400, 'validation_error');
    }
    ...
    if (role === 'admin' && req.user.role !== 'superadmin') {
      throw new ApiError('Only superadmins can create admin users', 403, 'forbidden');
    }
...
export const getUserStats = async (req, res, next) => {
  try {
    ...
    const kycRejectedUsers = await User.countDocuments({ kycStatus: 'rejected' });
