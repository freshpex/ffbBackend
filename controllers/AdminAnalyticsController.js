  ...
  try {
    ...
    const overview = {
      ...
      financial: {
        ...
        netBalance: (depositSum.length > 0 ? depositSum[0].total : 0) - 
...
export const getUserGrowthAnalytics = async (req, res, next) => {
  try {
    ...
    if (period === 'day') {
      ...
      startDate = start ? new Date(start) : new Date(new Date().setDate(endDate.getDate() - 30));
      timeGroup = { 
        $dateToString: { 
          format: '%Y-%m-%d', 
          date: '$createdAt' 
      ...
      startDate = start ? new Date(start) : new Date(new Date().setDate(endDate.getDate() - 84)); // 12 weeks
      timeGroup = { 
        $concat: [
      ...
      startDate = start ? new Date(start) : new Date(new Date().setMonth(endDate.getMonth() - 12));
      timeGroup = { 
        $dateToString: { 
          format: '%Y-%m', 
          date: '$createdAt' 
    ...
    const verifiedUsers = await User.countDocuments({ role: 'user', emailVerified: true });
    ...
    const userAnalytics = {
      ...
      conversion: {
        ...
        kycSubmissionRate: totalUsers > 0 ? (kycSubmitted / totalUsers) * 100 : 0,
        kycApprovalRate: kycSubmitted > 0 ? (kycApproved / kycSubmitted) * 100 : 0,
        depositConversionRate: totalUsers > 0 ? 
          ((usersWithDeposits[0]?.total || 0) / totalUsers) * 100 : 0
      }
    };
    ...
    financialData.forEach(item => {
      ...
      if (type) {
        ...
        if (type === 'deposit' || type === 'withdrawal') {
          ...
          periodsMap[period].revenue += parseFloat((item.total * feePercentage).toFixed(2));
        }
      }
    });
    
    // Convert map to array for response
    const timeSeriesData = Object.values(periodsMap).sort((a, b) => a.period.localeCompare(b.period));
...
export const getTransactionAnalytics = async (req, res, next) => {
  try {
    ...
    // Convert map to array for response
    const timeSeriesData = Object.values(periodMap).sort((a, b) => a.period.localeCompare(b.period));
...
export const getPerformanceAnalytics = async (req, res, next) => {
  try {
    ...
    const userRetentionRate = userRetentionAnalysis.length > 0 
      ? (userRetentionAnalysis[0].retainedUsers / userRetentionAnalysis[0].totalUsers) * 100 
      : 0;
    ...
    const performanceAnalytics = {
      ...
      operations: {
        kycProcessingTime: avgKycProcessingTime.length > 0 
          ? parseFloat(avgKycProcessingTime[0].avgTime.toFixed(2)) 
          : 0, // in hours
        ticketResponseTime: avgTicketResponseTime.length > 0 
          ? parseFloat(avgTicketResponseTime[0].avgTime.toFixed(2)) 
          : 0, // in minutes
        ticketResolutionTime: avgTicketResolutionTime.length > 0 
