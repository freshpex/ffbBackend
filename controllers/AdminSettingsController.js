...
export const getSettingsByCategory = async (req, res, next) => {
  try {
    ...
    logger.error(
      `Error fetching settings for category ${req.params.category}:`,
      error,
    );
    next(error);
  }
...
export const getSettingByKey = async (req, res, next) => {
  try {
    ...
    if (setting.sensitive && req.user.role !== "superadmin") {
      throw new ApiError(
        "Access denied to sensitive setting",
        403,
        "forbidden",
      );
    }
    ...
    for (const [key, value] of Object.entries(settings)) {
      try {
        ...
        if (setting.type === "select" && !setting.options.includes(value)) {
          errors.push(
            `Invalid value for '${key}'. Must be one of: ${setting.options.join(", ")}`,
          );
          continue;
        }
...
export const createSetting = async (req, res, next) => {
  try {
    const { key, value, description, category, type, options, sensitive } =
      req.body;

    // Only superadmins can create settings
    if (req.user.role !== "superadmin") {
      throw new ApiError(
        "Only superadmins can create new settings",
        403,
        "forbidden",
      );
    }
