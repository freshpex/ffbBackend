...
export const submitKyc = async (req, res, next) => {
  ...
  try {
    ...
    if (existingRequest) {
      throw new ApiError('You already have a pending KYC verification request', 400, 'duplicate_request');
    }
    
    // Validate required fields
    if (!firstName || !lastName || !dateOfBirth || !address || !city || !country || !documentType) {
      throw new ApiError('All required fields must be provided', 400, 'validation_error');
    }
    
    // Validate document uploads
    if (!req.files || !req.files.idDocument || !req.files.proofOfAddress) {
      throw new ApiError('ID document and proof of address are required', 400, 'validation_error');
    }
    
    // Upload documents to S3
    const idDocumentUrl = await uploadToS3(req.files.idDocument[0], 'kyc-documents');
    const proofOfAddressUrl = await uploadToS3(req.files.proofOfAddress[0], 'kyc-documents');
...
export const getKycStatus = async (req, res, next) => {
  try {
    ...
    const user = await User.findById(userId).select('kycStatus kycVerified kycApprovedAt');
