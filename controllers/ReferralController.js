...
export const applyReferralCode = async (req, res, next) => {
  ...
  try {
    ...
    if (!referralCode || !userId) {
      throw new ApiError('Referral code and user ID are required', 400, 'validation_error');
    }
    ...
    const existingReferral = await Referral.findOne({ referee: userId }).session(session);
    
    if (existingReferral) {
      throw new ApiError('User already has a referral', 400, 'duplicate_referral');
    }
...
export const completeReferral = async (req, res, next) => {
  ...
  try {
    ...
    if (referral.status !== 'pending') {
      throw new ApiError(`Referral is already ${referral.status}`, 400, 'invalid_status');
    }
    ...
    const referralProgram = {
      ...
      tiers: [
        ...
        },
        {
          ...
          benefits: ['25% increased referral bonus', 'Priority customer support']
        },
        {
          level: 3,
          name: 'Gold',
          ...
          maxReferrals: 30,
          bonusMultiplier: 1.5,
          benefits: ['50% increased referral bonus', 'Reduced trading fees', 'Premium educational content']
        },
        {
          level: 4,
          name: 'Platinum',
          ...
          maxReferrals: 50,
          bonusMultiplier: 2.0,
          benefits: ['100% increased referral bonus', 'VIP customer support', 'Exclusive investment opportunities']
        }
      ],
      steps: [
        {
        ...
        },
        {
          ...
          description: 'Share your referral link via email, social media, or messaging apps.'
        },
        {
          step: 3,
          title: 'Friends Sign Up',
          description: 'When friends use your link to sign up, they become your referrals.'
        },
        {
          step: 4,
          title: 'Earn Rewards',
          description: 'Both you and your friends earn bonuses once requirements are met.'
        }
      ]
    };
