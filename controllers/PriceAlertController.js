...
export const createPriceAlert = async (req, res, next) => {
  try {
    const userId = req.user._id;
    const { symbol, price, condition, repeatable, notificationMethods } = req.body;
    ...
    const newAlert = await PriceAlert.create({
      ...
      notificationMethods: notificationMethods || { app: true, email: false, sms: false }
    });
...
export const updatePriceAlert = async (req, res, next) => {
  try {
    ...
    const { symbol, price, condition, repeatable, active, notificationMethods } = req.body;
...
export const checkPriceAlerts = async (marketData) => {
  try {
    ...
    try {
      ...
      for (const alert of activeAlerts) {
        ...
        if (isTriggered) {
          ...
          if (alert.notificationMethods.app) {
            await createAlertNotification(user._id, alert, currentPrice, session);
          }
      ...
      logger.info(`Processed ${activeAlerts.length} alerts, triggered ${triggeredCount}`);
...
const createAlertNotification = async (userId, alert, currentPrice, session) => {
  try {
