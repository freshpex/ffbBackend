...
export async function getEmailTemplate(templateKey) {
  ...
  const fromDb =
    setting?.value && typeof setting.value === "object" ? setting.value : null;

  const resolved = {
    templateKey,
    description:
      (fromDb && fromDb.description) || defaultTemplate?.description || "",
    title:
      (fromDb && fromDb.title) ||
      defaultTemplate?.title ||
      (fromDb && fromDb.subject) ||
...
export async function upsertEmailTemplate({
  ...
  const hasPlainMessage = typeof message === "string" && message.trim();
  const hasHtml = typeof html === "string" && html.trim();
  const hasText = typeof text === "string" && text.trim();

  if (!hasPlainMessage && !hasHtml && !hasText) {
    throw new ApiError(
      "Template must include a message",
      400,
      "validation_error",
    );
  }
  ...
  const payload = {
    title: title || subject,
    subject,
    message: hasPlainMessage ? message : text || "",
    html: hasHtml
      ? html
      : buildBrandedEmailHtml({
          title: title || subject,
    ...
    text: hasText ? text : message || "",
    description:
      description || DEFAULT_EMAIL_TEMPLATES[templateKey]?.description || "",
  };
...
export async function sendEmail({
  ...
  if (!html && !text) {
    throw new ApiError(
      "Email html or text is required",
      400,
      "validation_error",
    );
  }
  ...
  const senderEmail =
    fromEmail || process.env.MAILJET_SENDER_EMAIL || "support@ffbroker.cam";
...
export async function listEmailTemplates() {
  ...
  const fromDbKeys = settings.map((s) =>
    s.key.replace(TEMPLATE_KEY_PREFIX, ""),
  );
