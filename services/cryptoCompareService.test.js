...
describe("CryptoCompare Service", () => {
  ...
  describe("executeRequest", () => {
    test("throws error when API key is not configured", async () => {
      delete process.env.CRYPTOCOMPARE_API_KEY;
      await expect(
        cryptoCompareService.executeRequest("test-endpoint"),
      ).rejects.toThrow("CryptoCompare API key is not configured");
    });
    ...
    test("calls API with correct parameters", async () => {
      ...
      expect(axios.get).toHaveBeenCalledWith(
        expect.stringContaining(
          "https://min-api.cryptocompare.com/data/endpoint?param=value",
        ),
    ...
    test("handles API error responses", async () => {
      const errorResponse = {
        data: { Response: "Error", Message: "API Error" },
      };
      callWithRetry.mockResolvedValue(errorResponse);

      await expect(
        cryptoCompareService.executeRequest("endpoint"),
  ...
  describe("API methods", () => {
    beforeEach(() => {
      ...
      cryptoCompareService.executeRequest = jest
        .fn()
    ...
    test("getMultipleCurrentPrices calls executeRequest with correct parameters", async () => {
      await cryptoCompareService.getMultipleCurrentPrices(
        ["BTC", "ETH"],
        ["USD", "EUR"],
      );
      expect(cryptoCompareService.executeRequest).toHaveBeenCalledWith(
    ...
    test("getHistoricalMinuteData calls executeRequest with correct parameters", async () => {
      ...
      expect(cryptoCompareService.executeRequest).toHaveBeenCalledWith(
        "histominute",
        { fsym: "BTC", tsym: "USD", limit: 60, aggregate: 5 },
      );
    });
