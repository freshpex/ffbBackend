...
describe('Alpha Vantage Service', () => {
	...
	describe('getExchangeRate', () => {
		it('should fetch exchange rate correctly', async () => {
			...
			expect(result).toEqual(mockResponse.data['Realtime Currency Exchange Rate']);
		});
	});
	...
	describe('error handling', () => {
		it('should handle API error messages', async () => {
			...
			await expect(alphaVantageService.getStockQuote('INVALID')).rejects.toThrow('Invalid API call');
		...
		it('should log warnings for API information messages', async () => {
			...
			expect(logger.warn).toHaveBeenCalledWith('Alpha Vantage API message: API call frequency limit reached');
		});

		it('should handle network errors', async () => {
			axios.get.mockRejectedValueOnce(new Error('Network error'));
			
			await expect(alphaVantageService.getStockQuote('AAPL')).rejects.toThrow('Network error');
			expect(logger.error).toHaveBeenCalled();
		});
	});
