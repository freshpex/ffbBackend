...
describe('marketDataService', () => {
  ...
  describe('getCurrentPrice', () => {
    ...
    test('should fallback to CryptoCompare when Binance fails', async () => {
      ...
      cryptoCompareService.getPrice.mockResolvedValue({ USD: 45000.50 });
    ...
    test('should throw error when all services fail', async () => {
      binanceService.getTickerPrice.mockRejectedValue(new Error('Binance API error'));
      cryptoCompareService.getPrice.mockRejectedValue(new Error('CryptoCompare API error'));
      
      await expect(marketDataService.getCurrentPrice('BTCUSDT')).rejects.toThrow('Could not fetch price for BTCUSDT');
    });
  });
  ...
  describe('getMarketData', () => {
    ...
    test('should handle errors for individual symbols', async () => {
      binanceService.getTickerPrice.mockResolvedValueOnce({ price: '45000.50' });
      binanceService.getTickerPrice.mockRejectedValueOnce(new Error('API error'));
      cryptoCompareService.getPrice.mockRejectedValueOnce(new Error('API error'));
      
      const result = await marketDataService.getMarketData(['BTCUSDT', 'ETHUSDT']);
  ...
  describe('getOHLCV', () => {
    test('should get OHLCV data for crypto from Binance', async () => {
      ...
      const result = await marketDataService.getOHLCV({ symbol: 'BTCUSDT', interval: '1h', limit: 10 });
      
      expect(binanceService.getKlines).toHaveBeenCalledWith({ symbol: 'BTCUSDT', interval: '1h', limit: 10 });
      expect(result).toEqual([{
    ...
    test('should fallback to CryptoCompare when Binance fails', async () => {
      ...
      cryptoCompareService.getHistoricalDailyData.mockResolvedValue([
        { time: 1625097600, open: 35000, high: 36000, low: 34500, close: 35800, volumeto: 1000 }
      ]);
      
      const result = await marketDataService.getOHLCV({ symbol: 'BTCUSDT', interval: '1h', limit: 10 });
      
      expect(cryptoCompareService.getHistoricalDailyData).toHaveBeenCalledWith('BTC', 'USD', 10);
    ...
    test('should get OHLCV data for stocks from AlphaVantage', async () => {
      ...
      const result = await marketDataService.getOHLCV({ symbol: 'AAPL', interval: '1d', limit: 10 });
      
      expect(alphaVantageService.getTimeSeries).toHaveBeenCalledWith('AAPL', 'daily', 10);
  ...
  describe('getOrderBook', () => {
    test('should get order book for crypto symbol', async () => {
      binanceService.getDepth.mockResolvedValue({
        bids: [['35000', '0.5'], ['34900', '1.2']],
        asks: [['35100', '0.3'], ['35200', '0.8']]
      });
    ...
    test('should throw error for non-crypto symbols', async () => {
      await expect(marketDataService.getOrderBook('AAPL')).rejects.toThrow('Order book unavailable for AAPL');
    });
  });
