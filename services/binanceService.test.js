...
describe('Public API methods', () => {
  ...
  test('getKlines returns kline data with required parameters', async () => {
    const mockResponse = [[1617667200000, "56000", "58000", "55900", "57500", "1000"]];
    axios.get.mockResolvedValue({ data: mockResponse });
  ...
  test('makePublicRequest handles API errors properly', async () => {
    ...
    await expect(binanceService.getTickerPrice('INVALID')).rejects.toThrow('Binance API Error: Invalid symbol');
  });
});
...
describe('Authenticated API methods', () => {
  ...
  test('getAccountInfo returns account data', async () => {
    const mockResponse = { makerCommission: 10, takerCommission: 10, balances: [] };
    axios.get.mockResolvedValue({ data: mockResponse });
  ...
  test('placeOrder creates a new order correctly', async () => {
    ...
    expect(axios.post).toHaveBeenCalledWith(
      ...
      expect.stringContaining('symbol=BTCUSDT&side=BUY&type=LIMIT&quantity=0.1&price=50000'),
      expect.objectContaining({
        headers: expect.objectContaining({ 'X-MBX-APIKEY': 'test-api-key' })
      })
    );
  ...
  test('cancelOrder cancels an existing order', async () => {
    const mockResponse = { orderId: 1234, symbol: 'BTCUSDT', status: 'CANCELED' };
  ...
  test('makeAuthenticatedRequest handles API errors properly', async () => {
    ...
    await expect(binanceService.getAccountInfo()).rejects.toThrow('Binance API Error: Invalid API key');
  });
});