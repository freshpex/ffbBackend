...
const checkAndCreateAdmin = async () => {
  try {
    ...
    if (usersWithNullUid.length > 0) {
      console.log(`Found ${usersWithNullUid.length} users with null UIDs. Fixing...`);
    ...
    if (adminCount > 0) {
      ...
      admins.forEach(admin => {
        console.log(`- ${admin.email} (${admin.firstName} ${admin.lastName}, ${admin.role}) UID: ${admin.uid || 'null'}`);
      });
      ...
      if (testAdmin) {
        console.log('Test admin already exists, updating password and ensuring UID is set...');
        ...
        if (!testAdmin.uid) {
          ...
          console.log(`Setting missing UID for existing admin: ${testAdmin.uid}`);
        }
        
        testAdmin.password = hashedPassword;
        await testAdmin.save();
        
        console.log(`Test admin account updated: ${testAdminEmail} / ${testAdminPassword}`);
      } else {
        ...
        try {
          ...
          await newAdmin.save();
          console.log(`Test admin account created successfully with UID: ${newAdmin.uid}`);
        } catch (createError) {
          console.error('Error creating test admin:', createError.message);
          
          if (createError.code === 11000) {
            console.log('Duplicate key error. Attempting with a different UID...');
            ...
            await newAdmin.save();
            console.log(`Test admin account created with fallback UID: ${fallbackUid}`);
          } else {
            throw createError;
          }
        }
      ...
      console.log(`Test admin account ready: ${testAdminEmail} / ${testAdminPassword}`);
    } else {
