...
export const connectDatabase = async (attempt = 1) => {
  try {
    ...
    logger.info(
      `Connecting to MongoDB... (Attempt ${attempt}/${MAX_CONNECTION_ATTEMPTS})`,
    );
    ...
    logger.error(
      `MongoDB connection error (Attempt ${attempt}/${MAX_CONNECTION_ATTEMPTS}):`,
      {
    ...
    if (attempt < MAX_CONNECTION_ATTEMPTS) {
      ...
      logger.error(
        `Failed to connect to MongoDB after ${MAX_CONNECTION_ATTEMPTS} attempts. Stopping retry attempts.`,
      );
      process.exit(1);
    }
  }
...
export const monitorDatabase = () => {
  ...
  mongoose.connection.on("error", (err) => {
    ...
    if (
      ...
      setTimeout(() => {
        connectDatabase().catch((reconnectErr) =>
          logger.error("MongoDB reconnection failed:", {
